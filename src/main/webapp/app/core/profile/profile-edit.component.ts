...
export default defineComponent({
  ...
  setup() {
    ...
    const initAuthorities = async () => {
      const response = await profileService.retrieveAuthorities();
  ...
  methods: {
    ...
    getToastMessageFromHeader(res: any): string {
      return this.t$(res.headers['x-szakdolgozatapp-alert'], {
        param: decodeURIComponent(res.headers['x-szakdolgozatapp-params'].replace(/\+/g, ' ')),
      }).toString();
    },
  },
});
